...
public class MainActivityListAdapter extends BaseExpandableListAdapter {
	...
	public View getChildView(int groupPosition, int childPosition,
		...
		case EVENTS_IDX:
			...
			View v = /* convertView != null ? convertView : */LayoutInflater
					.from(ctx).inflate(R.layout.item_event, null);
			Event e = (Event) getChild(groupPosition, childPosition);
			
			boolean isNew = ctx.getNewEventCount() > 0 ? e.getDate().getTime() >= ctx.getNewEventCutoff() : false;

			TextView dateView = (TextView) v.findViewById(R.id.eventDate);
			dateView.setText(DateUtils.getRelativeDateTimeString(ctx, e
					.getDate().getTime(), DateUtils.MINUTE_IN_MILLIS,
			...
			/*
			 * etypes: 1 = red 2,3 = orange 4,5 = grey otherwise green
			 */
			int color;
			switch (e.getEtype()) {
			case 1:
	...
	public View getGroupView(int groupPosition, boolean isExpanded,
		...
		switch (groupPosition) {
		case EVENTS_IDX:
			View v = LayoutInflater.from(ctx).inflate(R.layout.group_events,
					null);
